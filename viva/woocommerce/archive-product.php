...
$shop_pagination_design = viva_get_option( 'tek-woo-catalog-style' );
...
$shop_pagination_class = apply_filters( 'viva_shop_pagination_class', $shop_pagination_class );

$shop_columns = $shop_sidebar = $shop_cols_class = $shop_sidebar_cols_class ='';
if ( '' == viva_get_option( 'tek-woo-shop-columns' ) ) {
	$shop_columns = 'woo-3-columns';
} else {
	$shop_columns = apply_filters( 'viva_shop_columns', viva_get_option( 'tek-woo-shop-columns' ) );
}

if ( viva_get_option( 'tek-woo-sidebar-position' ) ) {
	$shop_sidebar = apply_filters( 'viva_shop_sidebar_position', viva_get_option( 'tek-woo-sidebar-position' ) );
}
