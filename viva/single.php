...
<?php $page_layout = $single_post_template = $sticky_sidebar = $post_header_image = $post_image_class = $blog_active_widgets = $blog_single_sidebar = '';
...
if ( viva_get_option( 'tek-blog-sticky-sidebar' ) ) {
...
if ( '' != viva_get_option( 'tek-single-post-template' ) && is_singular( 'post' ) ) {
	$single_post_template = apply_filters( 'viva_blog_single_template', viva_get_option( 'tek-single-post-template' ) );
} else {
...
<div id="posts-content" class="blog-single <?php echo esc_attr( $page_layout ); ?>">
	<?php while ( have_posts() ) : the_post(); ?>
		...
		<div class="container single-post-content">
			...
			<?php
			if ( viva_get_option( 'tek-related-posts' ) == true )  {
